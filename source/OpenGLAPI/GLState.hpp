...
namespace GLType
{
    ...
    const inline static std::array<std::unordered_map<ErrorType, std::vector<std::string>>, util::toIndex(Function::Count)> functionErrorTypeMapping =
    {{
        { // UniformBlockBinding
    ...
    inline std::string toString(const Function& pFunction)
    {
        // #C++20 - Migrate the arrays to constexpr
        // The following proposal to C++20 adds constexpr versions of constructors to std::string
        // https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2019/p0980r1.pdf
        // While doing this work, move all the string arrays into function scope as below to avoid pollution in debugging
        // + improve readibility/scope of usage
        // #TODO: Investigate why initialisation lists with less entries than size do not throw compile-time errors. (they instead initialise to empty strings)
        static const std::array<std::string, util::toIndex(Function::Count)> FunctionStrings =
            {"UniformBlockBinding",
             "Viewport",
             "DrawElements",
             "DrawArrays",
        ...
        return FunctionStrings[util::toIndex(pFunction)];
    }
