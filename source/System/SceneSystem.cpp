...
namespace System
{
	...
	void SceneSystem::update_scene_bounds()
	{
		...
		getCurrentScene().foreach([&scene = m_scene.m_entities, &scene_bounds = m_scene.m_bound](ECS::Entity p_entity, Component::Transform& p_transform, Component::Mesh& p_mesh)
		{
			...
			{
				//p_mesh.mModel->forEachMesh([&transform = p_transform, &scene_bounds = scene_bounds](const Data::Mesh& p_mesh)
				//{
				//    const auto world_AABB = p_mesh.mAABB.transform(p_mesh.mAABB, transform.mPosition, glm::mat4_cast(transform.mOrientation), transform.mScale);
	...
	void SceneSystem::primitives_scene()
	{
		{ // Plane/quad
			...
			auto mesh         = Component::Mesh{mMeshSystem.m_quad};
		...
		{ // Textured cube
			...
			auto mesh         = Component::Mesh{mMeshSystem.m_cube};
		...
		{ // Cone
			auto transform    = Component::Transform{glm::vec3(running_x, start_y, -mesh_width)};
			auto mesh         = Component::Mesh{mMeshSystem.m_cone};
		...
		{ // Cylinder
			auto transform    = Component::Transform{glm::vec3(running_x, start_y, -mesh_width)};
			auto mesh         = Component::Mesh{mMeshSystem.m_cylinder};
		...
		{ // Sphere
			auto transform    = Component::Transform{glm::vec3(running_x, start_y, -mesh_width)};
			auto mesh         = Component::Mesh{mMeshSystem.m_sphere};
	...
	void SceneSystem::constructBoxScene()
	{
		...
		{// Cubes
			for (size_t i = 0; i < 100; i += 2)
			{
				...
				Component::Mesh mesh          = Component::Mesh(mMeshSystem.m_cube);
	...
	void SceneSystem::constructBouncingBallScene()
	{
		...
		{ // Floor
			...
			Component::Mesh mesh = {mMeshSystem.m_plane};
			Component::Collider collider = Component::Collider(transform, mesh);
