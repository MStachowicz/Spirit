...
namespace System
{
	...
	std::vector<std::pair<ECS::Entity, float>> CollisionSystem::get_entities_along_ray(const Geometry::Ray& p_ray) const
	{
		...
		// m_scene_system.get_current_scene_entities().foreach([&](ECS::Entity& p_entity, Component::Terrain& p_terrain, Component::Transform& p_transform)
		// {
		// 	auto terrain_mesh_AABB = p_terrain.m_mesh.AABB;
		// 	terrain_mesh_AABB      = Geometry::AABB::transform(terrain_mesh_AABB, p_transform.m_position, glm::identity<glm::mat4>(), glm::vec3(1.f));
		// 	float length_along_ray = 0.f;
