        ...
        { // ECS UNIT TESTS
            ...
            {// getComponentMutable tests
             {
                ...
                { // TEST 3: Add delete add (overwrite previous data)
                    ...
                    count = 0;
                    storage.foreach ([&count](size_t& pComponent) { ZEPHYR_ASSERT(pComponent == count++, "deleteEntity: Missmatch value - Order didnt change, values in archetype should be 0-1-2"); });
            ...
            { // foreach with Entity
                ...
                { // TEST 1: Iterate exact match archetype and count the same unique set of entities returned
                    ...
                    ZEPHYR_ASSERT(entitySet.size() == 12, "foreach with Entitys failed");
                ...
                { // TEST 4: 1 entity
                    ...
                    storage.foreach ([&entitySet](ECS::Entity& pEntity, float& pFloat, double& pDouble, bool& pBool)
                    {
                        ...
                        ZEPHYR_ASSERT(pDouble == 2.0, "foreach with Entity: missmatch value");
                        ZEPHYR_ASSERT(pFloat == 4.f, "foreach with Entity: missmatch value");
                        ZEPHYR_ASSERT(pBool == false, "foreach with Entity: missmatch value");
                    });
